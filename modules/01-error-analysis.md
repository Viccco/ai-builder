...
4. Axial coding. Open `apps/01-clusters/index.html`. Your labels are already there. It shows you one failure at a time with everything you need to judge it, and you click the cause it belongs to or press its number. When nothing fits, name a new cause right there. `S` skips a card to the back of the queue if you want to see more before deciding. When everything is sorted you land on the board, where you write the one-sentence definitions, move anything you got wrong, and star the clearest example of each mode. Rule of thumb: 3 to 6 categories. (~10 min)
